...
/**
 * Global configuration for the Extended Email plugin, exposed as a standalone
...
public class ExtendedEmailGlobalConfiguration extends GlobalConfiguration {
    ...
    /**
     * Migrates the config file from the old {@code ExtendedEmailPublisherDescriptor.xml}
     * to {@code ExtendedEmailGlobalConfiguration.xml} by rewriting the root element name.
     * Runs before extensions are augmented so the new file is present when this class is loaded.
     */
    ...
    public static void migrateConfigFile() {
        ...
        try {
            String xml = new String(Files.readAllBytes(oldFile.toPath()), StandardCharsets.UTF_8);
            xml = xml.replace(
                    "<hudson.plugins.emailext.ExtendedEmailPublisherDescriptor>",
                    "<hudson.plugins.emailext.ExtendedEmailGlobalConfiguration>");
            xml = xml.replace(
                    "</hudson.plugins.emailext.ExtendedEmailPublisherDescriptor>",
                    "</hudson.plugins.emailext.ExtendedEmailGlobalConfiguration>");
            Files.write(newFile.toPath(), xml.getBytes(StandardCharsets.UTF_8));
            LOGGER.info("Migrated email-ext configuration to ExtendedEmailGlobalConfiguration.xml");
        } catch (IOException e) {
            LOGGER.log(Level.WARNING, "Failed to migrate email-ext global configuration", e);
        }
    }

    public ExtendedEmailGlobalConfiguration() {
        if (mailAccount == null) {
    ...
    public GlobalConfigurationCategory getCategory() {
        return GlobalConfigurationCategory.get(ExtendedEmailManagementLink.Category.class);
    }

    public static ExtendedEmailGlobalConfiguration get() {
        return GlobalConfiguration.all().getInstance(ExtendedEmailGlobalConfiguration.class);
    }
    ...
    public static void autoConfigure() {
        ...
        if (Jenkins.get().isUseSecurity()
                && (!StringUtils.isBlank(config.getDefaultPostsendScript())
                        || !StringUtils.isBlank(config.getDefaultPresendScript()))) {
            config.setDefaultPostsendScript(config.getDefaultPostsendScript());
            config.setDefaultPresendScript(config.getDefaultPresendScript());
            try {
                config.setDefaultClasspath(config.getDefaultClasspath());
            } catch (FormException e) {
                List<GroovyScriptPath> newList = new ArrayList<>();
                for (GroovyScriptPath path : config.getDefaultClasspath()) {
                    URL u = path.asURL();
                    if (u != null) {
                        try {
                ...
                try {
                    config.setDefaultClasspath(newList);
    ...
    FormValidation validateAttachmentPattern(String pattern) {
