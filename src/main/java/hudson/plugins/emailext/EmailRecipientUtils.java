...
public class EmailRecipientUtils {
    ...
    private static Set<InternetAddress> convertRecipientString(
            String recipientList, EnvVars envVars, int type, TaskListener listener)
            throws AddressException, UnsupportedEncodingException {
        ...
        if (!StringUtils.isBlank(recipientList)) {
            ...
            RecipientListStringAnalyser recipientsAnalyser =
                    RecipientListStringAnalyser.newInstance(listener, expandedRecipientList);
            InternetAddress[] all = InternetAddress.parse(
                    expandedRecipientList.replace("bcc:", "").replace("cc:", ""));
            ...
            for (InternetAddress address : internetAddresses) {
                if (!address.getAddress().contains("@")) {
                    ...
                    if (u != null) {
                        ...
                        if (userEmail != null) {
                            // if configured user email does not have @domain prefix, then default prefix will be added
                            // on next step
                ...
                if (address.getPersonal() != null) {
                    address.setPersonal(
                            address.getPersonal(),
                            ExtendedEmailPublisher.descriptor().getCharset());
                }
            }
        }
    ...
    public static boolean isAllowedDomain(String userName, TaskListener listener) {
        ...
        if (descriptor.getAllowedDomains() != null) {
            ...
            while (tokens.hasMoreTokens()) {
                ...
                descriptor.debug(
                        listener.getLogger(), "Checking '%s' against '%s' to see if they are allowed", userName, check);
    ...
    public static boolean isExcludedRecipient(String userName, TaskListener listener) {
        ...
        if (descriptor.getExcludedCommitters() != null) {
            ...
            while (tokens.hasMoreTokens()) {
                ...
                descriptor.debug(
                        listener.getLogger(),
                        "Checking '%s' against '%s' to see if they are excluded",
                        userName,
                        check);
    ...
    public static boolean isExcludedRecipient(User user, TaskListener listener) {
        ...
        String[] testValues = new String[] {
            user.getFullName(), user.getId(), user.getDisplayName(), prop != null ? prop.getAddress() : null
        };
    ...
    public static void addAddressesFromRecipientList(
        ...
        try {
            Set<InternetAddress> internetAddresses =
                    convertRecipientString(recipientList, envVars, EmailRecipientUtils.TO, listener);
            to.addAll(internetAddresses);
            if (bcc != null) {
                Set<InternetAddress> bccInternetAddresses =
                        convertRecipientString(recipientList, envVars, EmailRecipientUtils.BCC, listener);
                bcc.addAll(bccInternetAddresses);
            }
            if (cc != null) {
                Set<InternetAddress> ccInternetAddresses =
                        convertRecipientString(recipientList, envVars, EmailRecipientUtils.CC, listener);
    ...
    public static String getRecipientList(ExtendedEmailPublisherContext context, String recipients)
        ...
        return StringUtils.isBlank(recipients)
                ? ""
                : ContentBuilder.transformText(
                        recipients, context, context.getPublisher().getRuntimeMacros(context));
    }
}
