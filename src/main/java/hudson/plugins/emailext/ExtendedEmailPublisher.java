...
=======
	...
	@Deprecated
	public ExtendedEmailPublisher(String project_recipient_list, String project_content_type,
			String project_default_subject, String project_default_content, String project_attachments,
			String project_presend_script, int project_attach_buildlog, String project_replyto, String project_from,
			boolean project_save_output, List<EmailTrigger> project_triggers, MatrixTriggerMode matrixTriggerMode) {
	...
	@DataBoundConstructor
	@SuppressFBWarnings(value = "PA_PUBLIC_PRIMITIVE_ATTRIBUTE", justification = "TODO needs triage")
	public ExtendedEmailPublisher(String recipientList, String contentType, String defaultSubject,
			String defaultContent, String attachmentsPattern, String presendScript, int attachBuildLog, String replyTo,
			String from, boolean saveOutput, List<EmailTrigger> configuredTriggers, MatrixTriggerMode matrixTriggerMode,
			boolean disabled, List<GroovyScriptPath> classpath) {
		this.recipientList = recipientList;
		this.contentType = contentType;
		this.defaultSubject = defaultSubject;
		this.defaultContent = defaultContent;
	...
	private boolean _perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener,
		...
		for (String triggerName : replacedTriggers) {
			...
			listener.getLogger().println(
					"Trigger " + triggerName + " was overridden by another trigger and will not send an email.");
		}
					...
					if (prop != null) {
						...
						for (EmailTrigger trigger : prop.getTriggers()) {
							if (trigger.isPreBuild() == forPreBuild && trigger.trigger(build, listener)) {
								String tName = trigger.getDescriptor().getDisplayName();
								watcherTriggered.put(tName, trigger);
								listener.getLogger().println("Email was triggered for watcher '" + user.getDisplayName()
										+ "' for: " + tName);
								emailTriggered = true;
							}
						}
						...
						for (String triggerName : replacedTriggers) {
							watcherTriggered.removeAll(triggerName);
							listener.getLogger().println("Trigger " + triggerName
									+ " was overridden by another trigger and will not send an email.");
						}
	...
	boolean sendMail(ExtendedEmailPublisherContext context) {
		try {
			...
			if (!mailAccount.isValid()) {
				if (!mailAccount.isFromAddressValid()) {
					...
					if (mailAccount.isDefaultAccount()) {
						...
						debug(context.getListener().getLogger(),
								"Additional account has invalid from address " + mailAccount.getAddress());
					}
				} else if (!mailAccount.isSmtpServerValid()) {
					context.getListener().getLogger().println("Mail account has invalid SMTP server");
			...
			MimeMessage msg = createMail(context, fromAddress, session);
			debug(context.getListener().getLogger(), "Successfully created MimeMessage");
			if (getDescriptor().isThrottlingEnabled() && EmailThrottler.getInstance().isThrottlingLimitExceeded()
					&& !context.getTrigger().shouldBypassThrottling()) {
				context.getListener().getLogger().println("Could not send email. Throttling limit exceeded.");
				return false;
			}
					...
					while (true) {
						try {
							...
							if (e.getNextException() != null && (e.getNextException() instanceof SocketException
								...
								Exception next = e.getNextException();
								while (next != null) {
									debug(context.getListener().getLogger(), "Next " + next.getClass().getSimpleName()
			...
			Functions.printStackTrace(e,
					context.getListener().error("SMTP authentication failed. Check mail credentials."));
		} catch (SendFailedException e) {
			LOGGER.log(Level.WARNING, "Email sending failed due to invalid or rejected recipient addresses.", e);
			Functions.printStackTrace(e,
					context.getListener().error("Email sending failed due to invalid recipient address."));
		} catch (MessagingException e) {
			LOGGER.log(Level.WARNING, "SMTP communication error while sending email.", e);
			Functions.printStackTrace(e, context.getListener().error("SMTP communication error while sending email."));
		} catch (Exception e) {
	...
	private void executePostsendScript(ExtendedEmailPublisherContext context, MimeMessage msg, Session session,
			Transport transport) throws RuntimeException {
		executeScript(postsendScript, "post-send", context, msg, session, transport);
	}
		...
		if (StringUtils.isNotBlank(script)) {
			...
			try {
				...
				if (AbstractEvalContent.isApprovedScript(script, GroovyLanguage.get())
					...
					GroovyShell shell = new GroovyShell(cl, binding, getCompilerConfiguration(true));
					try {
						new GroovySandbox().withWhitelist(new ProxyWhitelist(Whitelist.all(),
								new MimeMessageInstanceWhitelist(msg), new PropertiesInstanceWhitelist(props),
								new TaskListenerInstanceWhitelist(listener), new PrintStreamInstanceWhitelist(logger),
								new EmailExtScriptTokenMacroWhitelist())).runScript(shell, script);
						cancel = (Boolean) shell.getVariable("cancel");
					} catch (RejectedAccessException x) {
						throw ScriptApproval.get().accessRejected(x, ApprovalContext.create());
					}
	...
	private void transformToClasspathEntries(List<GroovyScriptPath> input, ExtendedEmailPublisherContext context,
			List<ClasspathEntry> output) {
		for (GroovyScriptPath path : input) {
			URL url = path.asURL();
			if (url != null) {
				try {
					...
					context.getListener().getLogger().printf(
							"[email-ext] Warning: Ignoring classpath: [%s] as it could not be transformed into a valid URL%n",
							path.getPath());
				}
			}
	...
	MailAccount getMailAccount(ExtendedEmailPublisherContext context) throws AddressException {
		...
		for (MailAccount addAccount : descriptor.getAddAccounts()) {
			...
			if (!addAccount.getAddress().equalsIgnoreCase(fromAddress.getAddress())) {
				debug(context.getListener().getLogger(), "Ignoring valid additional account " + addAccount.getAddress()
						+ " because it is not a match for " + from);
				continue;
			}
	...
	private MimeMessage createMail(ExtendedEmailPublisherContext context, InternetAddress fromAddress, Session session)
		...
		if (attachBuildLog || context.getTrigger().getEmail().getAttachBuildLog()) {
			debug(context.getListener().getLogger(), "Request made to attach build log");
			AttachmentUtils.attachBuildLog(context, multipart,
					compressBuildLog || context.getTrigger().getEmail().getCompressBuildLog());
		}
		...
		if (StringUtils.isNotBlank(emergencyReroute)) {
			debug(context.getListener().getLogger(), "Emergency reroute turned on");
			EmailRecipientUtils.addAddressesFromRecipientList(to, cc, bcc, emergencyReroute, env,
					context.getListener());
			debug(context.getListener().getLogger(), "Emergency reroute is set to: " + emergencyReroute);
		} else {
			for (RecipientProvider provider : context.getTrigger().getEmail().getRecipientProviders()) {
			...
			descriptor.debug(context.getListener().getLogger(), "Adding recipients from trigger recipient list");
			EmailRecipientUtils.addAddressesFromRecipientList(to, cc, bcc,
					EmailRecipientUtils.getRecipientList(context, context.getTrigger().getEmail().getRecipientList()),
					env, context.getListener());
		}
		...
		if (StringUtils.isNotBlank(replyTo)) {
			EmailRecipientUtils.addAddressesFromRecipientList(replyToAddresses, null, null,
					EmailRecipientUtils.getRecipientList(context, replyTo), env, context.getListener());
		}

		if (StringUtils.isNotBlank(context.getTrigger().getEmail().getReplyTo())) {
			EmailRecipientUtils.addAddressesFromRecipientList(replyToAddresses, null, null,
					EmailRecipientUtils.getRecipientList(context, context.getTrigger().getEmail().getReplyTo()), env,
					context.getListener());
		}
	...
	private void setSubject(ExtendedEmailPublisherContext context, MimeMessage msg, String charset)
			throws MessagingException {
		String subject = ContentBuilder.transformText(context.getTrigger().getEmail().getSubject(), context,
				getRuntimeMacros(context));
		msg.setSubject(subject, charset);
	}
	...
	private Multipart addContent(ExtendedEmailPublisherContext context, String charset) throws MessagingException {
		final String text = ContentBuilder.transformText(context.getTrigger().getEmail().getBody(), context,
				getRuntimeMacros(context));
		final Multipart multipart;
		boolean doBoth = false;

		String messageContentType = context.getTrigger().getEmail().getContentType().equals("project") ? contentType
				: context.getTrigger().getEmail().getContentType();
		// contentType is null if the project was not reconfigured after upgrading.
		if (messageContentType == null || "default".equals(messageContentType)) {
			messageContentType = getDescriptor().getDefaultContentType();
			...
			if (saveOutput) {
				...
				if (workspace != null) {
					FilePath savedOutput = new FilePath(workspace,
							"%s-%s%s".formatted(context.getTrigger().getDescriptor().getDisplayName(),
									context.getRun().getId(), extension));
					savedOutput.write(text, charset);
				} else {
					context.getListener().getLogger().println("No workspace to save the email to");
				}
	...
	public static @CheckForNull Run<?, ?> getPreviousRun(@NonNull Run<?, ?> run, TaskListener listener) {
		...
		if (previousRun != null && previousRun.isBuilding()) {
			listener.getLogger().println(Messages
					.ExtendedEmailPublisher__is_still_in_progress_ignoring_for_purpo(previousRun.getDisplayName()));
			return null;
		} else {
			return previousRun;
		}
	...
	public static final class MatrixAggregatableImpl implements MatrixAggregatable {
		@Override
		public MatrixAggregator createAggregator(MatrixBuild matrixbuild, Launcher launcher,
