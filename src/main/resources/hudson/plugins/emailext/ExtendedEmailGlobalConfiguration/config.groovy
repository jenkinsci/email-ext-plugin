...
f.advanced {
  ...
  f.entry(title: _("Additional accounts")) {
    f.repeatableProperty(field: "addAccounts", header: _("Account")) {
      f.block {
        div(align: "right") {
...
f.entry(field: "maxAttachmentSizeMb", help: "/plugin/email-ext/help/globalConfig/maxAttachmentSize.html", title: _("Maximum Attachment Size")) {
  f.number(checkUrl: "${rootURL}/${descriptor.descriptorUrl}/maxAttachmentSizeCheck", checkDependsOn: "")
}

f.entry(field: "defaultAttachBuildLogMode", help: "/plugin/email-ext/help/globalConfig/defaultAttachBuildLog.html", title: _("Default Attach Build Log")) {
  f.enum(default: "NONE") {
    raw(my.description)
  }
}
...
f.advanced(title: _("Default Triggers"), help: "/plugin/email-ext/help/globalConfig/defaultTriggers.html") {
  f.entry(title: "") {
    instance.triggerDescriptors.each { triggerDescriptor ->
      f.checkbox(name: "_.defaultTriggerIds", title: triggerDescriptor.displayName, checked: descriptor.defaultTriggerIds.contains(triggerDescriptor.id), json: triggerDescriptor.id)
      br()
    }
  }
}
