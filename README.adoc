...
== OAuth 2.0 Authentication for Microsoft Outlook (GSoC 2026)

Microsoft has deprecated Basic Authentication (username/password) for Exchange Online
and Outlook SMTP since September 2025, causing Jenkins email notifications to silently
fail for Microsoft 365 users.

This plugin is adding native OAuth 2.0 Client Credentials Flow support as part of
https://www.jenkins.io/projects/gsoc/2026/project-ideas/jenkins-email-notifications-using-outlook-smtp-with-oath/[Google Summer of Code 2026].

Once complete, Jenkins will authenticate to Outlook SMTP using short-lived Bearer tokens
from Microsoft Entra ID — no passwords stored, no Basic Auth required.

*Status:* Foundation classes in development (GSoC 2026, contributor: Murthi Haari).
...
Previous:: Add this recipient provider to send an email to the culprits, requestor and developers of the previous build(s).
...
[source,groovy]
----
// only send an email if the build failed and 'mickeymouse' had a commit
...
The *Default Subject* and *Default Content* fields on the project configuration page default to `$DEFAULT_SUBJECT` and `$DEFAULT_CONTENT` (respectively), delegating to the system-wide configuration from the *Extended E-mail Notification* section of the *Configure System* page.
...
There are two default scripts and templates available out of the box; one is designed for HTML emails and the other is designed for text emails.
